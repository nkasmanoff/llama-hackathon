...
class MoE(nn.Module):
    ...
    def __call__(self, x) -> mx.array:
        logits = self.router(
            x
        )  # linear layer shape is batch x num_tokens x num_experts
        print("LOGITS SHAPE", logits.shape)  # batch by tokens by num_experts
        # make the max value of all logits index 0
        # set max value of each row to element 0
        for i in range(logits.shape[1]):
            for j in range(logits.shape[2]):
                if j == 0:
                    ...
                    logits[0, i, j] = 0
        print("LOGITS MAXED", logits)
        k = self.top_k  # always 1
        # indices = [[0] * logits.shape[0]]
        # k = 1
...
class TransformerBlock(nn.Module):
    ...
    def __call__(
        ...
        print(f"You are now in layer {self.layer_idx}")
